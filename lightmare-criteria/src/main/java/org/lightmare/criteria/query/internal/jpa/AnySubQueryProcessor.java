...
import org.lightmare.criteria.query.internal.jpa.subqueries.SubQueryProvider.AnyQueryStream;
...
interface AnySubQueryProcessor<T> extends SubQueryOperator<T> {
    ...
    default <F, S> QueryStream<T> equal(EntityField<T, F> field, AnyQueryStream<S> stream) {
        return operateSubQuery(field, Operators.EQ, stream);
    }

    default <F, S> QueryStream<T> notEqual(EntityField<T, F> field, AnyQueryStream<S> stream) {
    ...
    default <F extends Comparable<? super F>, S> QueryStream<T> greaterThenOrEqualTo(
            EntityField<T, Comparable<? super F>> field, AnyQueryStream<S> stream) {
