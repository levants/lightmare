...
interface AllSubQueryProcessor<T> extends SubQueryOperator<T> {
     ...
     */
    default <F, S> QueryStream<T> operateSubQuery(EntityField<T, F> field, String operator, All<S> stream) {
    ...
    default <F, S> QueryStream<T> equal(EntityField<T, F> field, All<S> stream) {
        return operateSubQuery(field, Operators.EQ, stream);
    }

    default <F, S> QueryStream<T> notEqual(EntityField<T, F> field, All<S> stream) {
    ...
    default <F extends Comparable<? super F>, S> QueryStream<T> greaterThenOrEqualTo(
            EntityField<T, Comparable<? super F>> field, All<S> stream) {
