...
/**
 * Sub query processor for SOME clause and functional expressions
 * 
 ...
 */
interface SomeToFunctionSubQueryProcessor<T> extends SubQueryOperator<T> {
    ...
    default <F, S> QueryStream<T> equalSubQuery(FunctionConsumer<T> consumer, Some<S> stream) {
        return operateFunctionWthSubQuery(consumer, Operators.EQ, stream);
    }

    default <F, S> QueryStream<T> notEqualSubQuery(FunctionConsumer<T> consumer, Some<S> stream) {
    ...
    default <F extends Comparable<? super F>, S> QueryStream<T> greaterThenOrEqualToSubQuery(
            FunctionConsumer<T> consumer, Some<S> stream) {
        return operateFunctionWthSubQuery(consumer, Operators.GREATER_OR_EQ, stream);
    }
