package org.lightmare.jpa.packaging;
...
public final class PersistenceXmlLoaderImpl {
    ...
    private PersistenceXmlLoaderImpl() {
    }

    private static Document loadURL(URL configURL, EntityResolver resolver)
	    throws Exception {
	/*
	 * try and parse the document: - try and validate the document with
	 * persistence_2_0.xsd - if it fails because of the version attribute
	 * mismatch, try and validate the document with persistence_1_0.xsd
	 */
	InputStream is = null;
	if (configURL != null) {
	...
	if (is == null) {
	    throw new IOException(
		    "Failed to obtain InputStream while reading persistence.xml file: "
			    + configURL);
	}
	...
	if (errors.size() != 0) {
		...
		errorMessage.append("Error parsing XML (line")
			.append(error.getLineNumber()).append(" : column ")
			.append(error.getColumnNumber()).append("): ")
	...
	InputStream dtdStream = PersistenceXmlLoader.class.getClassLoader()
    ...
    public static List<PersistenceMetadata> deploy(URL url,
	    @SuppressWarnings("rawtypes") Map overrides, EntityResolver resolver)
	    throws Exception {
    ...
    /**
     * Method used by JBoss EJB3 (4.2 and above) for parsing Object used by
     * Hibernate OGM as well, consider this some kind of exposed service at the
     * SPI level
     */
    public static List<PersistenceMetadata> deploy(URL url,
	    @SuppressWarnings("rawtypes") Map overrides,
	...
	for (int i = 0; i < children.getLength(); i++) {
		...
		if (tag.equals("persistence-unit")) {
		    ...
		    /*
		     * if explicit => use it if JTA DS => JTA transaction if non
		     * JTA DA => RESOURCE_LOCAL transaction else default JavaSE
		     * => RESOURCE_LOCAL
		     */
		    PersistenceUnitTransactionType transactionType = metadata
			    .getTransactionType();
		    Boolean isJTA = null;
	...
	for (int i = 0; i < children.getLength(); i++) {
		...
		if (tag.equals("non-jta-data-source")) {
		    metadata.setNonJtaDatasource(XmlHelper
			    .getElementContent(element));
		} else if (tag.equals("jta-data-source")) {
		    metadata.setJtaDatasource(XmlHelper
			    .getElementContent(element));
		} else if (tag.equals("provider")) {
		    metadata.setProvider(XmlHelper.getElementContent(element));
		} else if (tag.equals("class")) {
		    metadata.getClasses().add(
			    XmlHelper.getElementContent(element));
		} else if (tag.equals("mapping-file")) {
		    metadata.getMappingFiles().add(
			    XmlHelper.getElementContent(element));
		} else if (tag.equals("jar-file")) {
		    metadata.getJarFiles().add(
			    XmlHelper.getElementContent(element));
		} else if (tag.equals("exclude-unlisted-classes")) {
		    String excludeText = XmlHelper.getElementContent(element);
		    if (excludeText != null) {
			boolean exclude = Boolean.valueOf(excludeText);
			metadata.setExcludeUnlistedClasses(exclude);
		    } else {
		    ...
		    metadata.setValidationMode(XmlHelper
			    .getElementContent(element));
		} else if (tag.equals("shared-cache-mode")) {
		    metadata.setSharedCacheMode(XmlHelper
			    .getElementContent(element));
			...
			if (props.item(j).getNodeType() == Node.ELEMENT_NODE) {
				...
				// description
				propValue = XmlHelper.getElementContent(
					propElement, "");
			    }
	...
	public void error(SAXParseException error) {
	    ...
	    // if ( ( (EJB3DTDEntityResolver) resolver ).isResolved() == false )
	    // return;
	    // }
	    errors.add(error);
	}
