...
public class ConnectionContainer {
    ...
    private static boolean checkOnProgress(ConnectionSemaphore semaphore) {
        return semaphore.isInProgress() && Boolean.FALSE.equals(semaphore.isBound());
    }
     ...
     */
    private static ConnectionSemaphore checkAndGetSemaphore(ConnectionSemaphore current,
     ...
     * @param semaphore
     */
    private static void cacheIfNot(String jndiName, ConnectionSemaphore semaphore) {

        if (StringUtils.valid(jndiName)) {
            ConnectionSemaphore existent = CONNECTIONS.putIfAbsent(jndiName, semaphore);
            if (existent == null) {
     ...
     * @return {@link ConnectionSemaphore}
     */
    public static ConnectionSemaphore cacheSemaphore(String unitName, String jndiName) {
     ...
     * @throws IOException
     */
    public static ConnectionSemaphore getConnection(String unitName) throws IOException {
     ...
     * @throws IOException
     */
    public static EntityManagerFactory getEntityManagerFactory(String unitName) throws IOException {
    ...
    private static void unbindConnection(ConnectionSemaphore semaphore) {
        ...
        if (ObjectUtils.notNull(jndiName) && semaphore.isBound()) {
            try {
                ...
                LogUtils.error(LOG, ex, NamingUtils.COULD_NOT_UNBIND_NAME_ERROR, jndiName, ex.getMessage());
            }
        }
    }
     ...
     * @param type
     */
    public static void setPollProviderType(String jndiName, PoolProviderType type) {
